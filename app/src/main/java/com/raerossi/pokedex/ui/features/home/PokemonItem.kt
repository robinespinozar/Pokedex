...
fun CardContent(
  ...
  /*      val context = LocalContext.current
        val bitmap = remember { BitmapFactory.decodeResource(context.resources, R.drawable.green_hoodie) }
        val palette = remember { Palette.from(bitmap).generate() }

        val darkVibrantSwatch = palette.darkVibrantSwatch
        val backgroundColor = darkVibrantSwatch?.let { Color(it.rgb) } ?: Color.White
*/
        AsyncImage(
            modifier = Modifier
                .fillMaxWidth()
